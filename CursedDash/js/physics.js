...
// js/physics.js - Часть 3 из 4
        ...
        // ИСПРАВЛЕНИЕ БАТУТОВ: Теперь они подбрасывают и катящегося по полу игрока
        for (let i = window.Game.pads.length - 1; i >= 0; i--) {
            ...
            // Расширили зону детекции: срабатывает и при падении, и при простом наступании на пол
    ...
    updateParticles() { 
        for (let i = window.Game.particles.length - 1; i >= 0; i--) { 
            ...
            p.life--; 
            p.element.style.left = p.x + 'px'; p.element.style.top = (400 - p.y) + 'px'; p.element.style.opacity = p.life / p.maxLife; p.element.style.width = p.size + 'px'; p.element.style.height = p.size + 'px';
