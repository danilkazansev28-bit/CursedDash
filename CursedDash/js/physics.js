...
// js/physics.js - Часть 2 из 4
    update() {
        ...
        // РАСЧЕТ ПУЛЬСАЦИИ ФОНА: Плавно уменьшаем яркость вспышки в каждом кадре
        ...
        if (liveContainer && window.Game.currentMusicFreq > 0) {
            // Рассчитываем оттенок синего/фиолетового цвета в зависимости от частоты ноты
            let hue = Math.floor((window.Game.currentMusicFreq % 200) + 200); 
            let brightness = Math.floor(8 + window.Game.bgPulseIntensity * 16); // Вспышка до 24% яркости, затухание до 8%
            liveContainer.style.backgroundColor = `hsl(${hue}, 65%, ${brightness}%)`;
        } else if (liveContainer) {
            liveContainer.style.backgroundColor = '#0a0813'; // Стандартный темный фон
        }
...
// js/physics.js - Часть 3 из 4
        ...
        for (let i = window.Game.pads.length - 1; i >= 0; i--) {
            ...
            if (cR > pd.x && cL < pd.x + pd.width && cB <= pd.bottom + pd.height + 4 && cT >= pd.bottom) {
    ...
    updateParticles() { 
        for (let i = window.Game.particles.length - 1; i >= 0; i--) { 
            ...
            p.life--; p.element.style.left = p.x + 'px'; p.element.style.top = (400 - p.y) + 'px'; p.element.style.opacity = p.life / p.maxLife; p.element.style.width = p.size + 'px'; p.element.style.height = p.size + 'px';
...
// js/physics.js - Часть 4 из 4
    ...
    createRocketTrail(x, y) {
        const liveContainer = document.getElementById('gameContainer'); if (!liveContainer || Math.random() > 0.4) return; const skinColor = window.Game.SKINS[window.Game.selectedSkinIndex].bg;
        ...
        if (Math.random() > 0.5) { const dEl = document.createElement('div'); dEl.className = 'particle'; dEl.style.background = 'rgba(150, 150, 150, 0.4)'; dEl.style.borderRadius = '50%'; liveContainer.appendChild(dEl); window.Game.particles.push({ element: dEl, x: x, y: y + 15, vx: -Math.random() * 2 - 1, vy: Math.random() * 0.5, life: 30, maxLife: 30, size: 8, isSmoke: true }); }
    },
