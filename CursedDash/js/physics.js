...
// js/physics.js - Часть 2 из 4
    update() {
        ...
        if (window.Game.currentMode === 'cube') {
            ...
            if (window.Game.isHoldingAction) { window.Game.cubeVelocityY += window.Game.THRUST_SHIP; } else { window.Game.cubeVelocityY += window.Game.GRAVITY_SHIP; }
...
// js/physics.js - Часть 3 из 4
    ...
    updateParticles() { 
        for (let i = window.Game.particles.length - 1; i >= 0; i--) { 
            const p = window.Game.particles[i]; p.x += p.vx; p.y += p.vy; p.vy += 0.2; p.life--; p.element.style.left = p.x + 'px'; p.element.style.top = (400 - p.y) + 'px'; p.element.style.opacity = p.life / 30; 
...
// js/physics.js - Часть 4 из 4
        ...
        if (window.Game.isTestingCustom) { 
            ...
            if (liveObjLayer) {
                ...
                window.Game.customObjects.forEach(obj => { 
                    ...
                    if (obj.type === 'spike-floor' || obj.type === 'spike-ceil') window.Game.spikes.push({ element: elClone, type: obj.type, x: obj.x, width: obj.width, height: obj.height, bottom: obj.bottom }); 
                    ...
                    else if (obj.type === 'solid-block') window.Game.solidBlocks.push({ element: elClone, x: obj.x, width: 40, height: 40, bottom: obj.bottom });
                }); 
            }
        }
    ...
    createExplosion(x, y) { const clr = window.Game.SKINS[window.Game.selectedSkinIndex].bg, liveContainer = document.getElementById('gameContainer'); if (!liveContainer) return; for (let i = 0; i < 20; i++) { const pEl = document.createElement('div'); pEl.classList.add('particle'); pEl.style.background = clr; liveContainer.appendChild(pEl); const angle = Math.random() * Math.PI * 2, speed = Math.random() * 5 + 2; window.Game.particles.push({ element: pEl, x: x + window.Game.CUBE_SIZE / 2, y: y + window.Game.CUBE_SIZE / 2, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, life: Math.random() * 20 + 20 }); } },
