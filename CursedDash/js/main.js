...
window.MenuEngine = {
    ...
    switchScreen(activeScreenKey) {
        ...
        for (let key in allScreens) {
            if (allScreens[key]) { allScreens[key].style.display = (key === activeScreenKey) ? 'flex' : 'none'; }
        }
    ...
    gameOver() { 
        ...
        if (window.AudioEngine) window.AudioEngine.playDeathSound(); 
...
/* js/main.js - Часть 2 из 2 */
    startGame(lvl) { 
        ...
        this.initDOMRefs(); window.PhysicsEngine.resetGame(); 
    },
    startCustomTest() { 
        this.switchScreen('none'); window.Game.isEditorMode = false; window.Game.isTestingCustom = true; window.Game.isHoldingAction = false; window.Game.currentLevel = 'custom'; 
        if (window.Game.DOM.scoreBoard) window.Game.DOM.scoreBoard.style.display = 'block'; 
    ...
    backToMenu() { 
        this.initDOMRefs(); if (window.Game.DOM.scoreBoard) window.Game.DOM.scoreBoard.style.display = 'none'; if (window.Game.DOM.stopTestBtn) window.Game.DOM.stopTestBtn.style.display = 'none'; if (window.Game.DOM.progressBarContainer) window.Game.DOM.progressBarContainer.style.display = 'none'; window.PhysicsEngine.clearGameContainer(); window.Game.isHoldingAction = false;
        if (window.Game.isTestingCustom) { window.Game.isTestingCustom = false; window.EditorEngine.openEditor(); if (window.Game.DOM.cube) window.Game.DOM.cube.style.display = 'none'; } 
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (publishBtn) { publishBtn.style.display = (urlParams.get('admin') === 'mysecret123') ? 'block' : 'none'; }
    ...
    safeBind('btnOpenEditor', () => { window.EditorEngine.openEditor(); if (window.Game.DOM.cube) window.Game.DOM.cube.style.display = 'none'; });
    safeBind('btnOpenSkins', () => window.MenuEngine.switchScreen('skins')); 
    safeBind('btnCloseSkins', () => window.MenuEngine.switchScreen('mainMenu')); 
    safeBind('btnStartTest', () => window.MenuEngine.startCustomTest());
    safeBind('btnSaveLevel', () => window.EditorEngine.saveCustomLevelPrompt());
    safeBind('btnClearLevel', () => window.EditorEngine.clearCustomLevel());
    safeBind('btnEditExit', () => window.MenuEngine.backToMenu());
