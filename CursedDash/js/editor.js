...
window.EditorEngine = {
    ...
    // Перерисовывает объекты с учетом текущего сдвига камеры (editorScrollX)
    updateEditorView() {
    ...
    setTool(tool) {
        ...
        const toolsMap = {
            ...
            'portal': 'toolPortal', 'orb-purple': 'toolOrbPurple', 'orb-pink': 'toolOrbPink', 
            'orb-red': 'toolOrbRed', 'pad-yellow': 'toolPadYellow', 'pad-pink': 'toolPadPink', 
            'pad-red': 'toolPadRed', 'speed-slow': 'toolSlow', 'speed-normal': 'toolNorm', 'speed-fast': 'toolFast', 'eraser': 'toolEraser'
        };
...
// js/editor.js - Часть 2 из 2
    initEditorEvents() {
        ...
        // Подключаем кнопки прокрутки камеры
        const scrLeft = document.getElementById('btnScrollLeft');
        const scrRight = document.getElementById('btnScrollRight');
        ...
        window.Game.DOM.container.addEventListener('mousedown', (e) => {
            ...
            if (window.Game.isMouseOverPanel || e.target === window.Game.DOM.stopTestBtn || e.target === window.Game.DOM.customTrackSelect || e.target.closest('#editorPanel')) return;
            
            const rect = window.Game.DOM.container.getBoundingClientRect();
            let clickX = e.clientX - rect.left;
            let clickY = e.clientY - rect.top;

            // Учитываем сдвиг камеры, чтобы объект сохранялся на глобальной карте
            let globalX = clickX + window.Game.editorScrollX;

            // МЕХАНИКА GRID SNAP (Сетка 40x40 пикселей)
            let snapX = Math.floor(globalX / 40) * 40;
            let snapY = Math.floor((400 - clickY) / 40) * 40;
            ...
            // Позиционируем элемент на экране с учетом сетки
            ...
            window.Game.customObjects.push({ element: newEl, type: type, x: snapX, bottom: snapY, width: width, height: height });
        });
    ...
    saveCustomLevelPrompt() {
        ...
        window.MenuEngine.renderSavedLevels(); alert("Сохранено!");
    },
