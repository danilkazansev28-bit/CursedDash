...
window.EffectsEngine = {
    // Красивое затухание неонового фона под музыку
    ...
    // Движение, уменьшение и исчезновение искр/дыма
    updateParticles() {
    ...
    // Выхлоп искр и серого дыма из ракеты
    createRocketTrail(x, y) {
        const liveContainer = document.getElementById('gameContainer');
        if (!liveContainer || Math.random() > 0.4) return;
